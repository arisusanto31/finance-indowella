    ...
    <div class="d-flex flex-column align-items-end">
        <div class="d-flex align-items-center gap-2">
            <label class="small text-muted mb-0">Tanggal:</label>
             <input type="date" name="tanggal_global"
                   class="form-control form-control-sm"
            ...
            <table class="table table-bordered table-striped table-hover align-middle">
                ...
                <tbody>
                    @foreach ($data['details'] as $i => $item)
                        <tr>
                            ...
                            <td>
                                <input type="number" name="quantity[{{ $i }}]" class="form-control qty" value="{{ $item->quantity }}">
                            </td>
                            <td>
                                <input type="number" name="price[{{ $i }}]" class="form-control price" value="{{ $item->price }}">
                            </td>
                            <td>
                                <input type="number" name="discount[{{ $i }}]" class="form-control disc" value="{{ $item->discount }}">
                            </td>
                            <td>
                                <input type="text" class="form-control total-field" value="{{ number_format(($item->quantity * $item->price) - $item->discount, 0, ',', '.') }}" readonly>
                            </td>
                        </tr>
                    @endforeach
                </tbody>
                <tfoot>
                    <tr>
                        ...
                        <td><input type="text" id="grand-total" class="form-control text-end" value="{{ format_price($data->total_price) }}" readonly></td>
                    </tr>
                </tfoot>
            </table>
        </div>
...
<script>
    ...
    const invoiceNumber = $('input[name="sales_order_number"]').val();
