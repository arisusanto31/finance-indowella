<x-app-layout>
    ...
    <div class="card shadow-sm mb-4">
        <h5 class="text-primary-dark card-header" style="padding-bottom:0px;"> 💳 <strong>KARTU IN TRANSIT</strong>
    ...
    @push('scripts')
        <script>
            ...
            function getSummary() {
                page = "kartu";
                $.ajax({
                    url: "{{ route('kartu-in-transit.get-summary') }}?month={{ $month }}&year={{ $year }}",
                    method: "GET",
                    success: function(res) {
                        console.log(res);
                        if (res.status == 1) {
                            ...
                            `;
                                ...
                                <td>${formatRupiah(item.saldo_rupiah_awal)}</td>
                                ...
                                <td>${formatRupiah(item.saldo_qty_akhir/item.konversi)} ${item.unit_default}</td>
            ...
            function showModalMasuk() {
                showDetailOnModal(
                    "{{ route('kartu-in-transit.create-mutasi-masuk') }}?month={{ $month }}&year={{ $year }}");
            }

            function showModalOut() {
                showDetailOnModal(
                    "{{ route('kartu-in-transit.create-mutasi-keluar') }}?month={{ $month }}&year={{ $year }}");
            }

            function getMutasiMasuk() {
                page = "masuk";
                $.ajax({
                    url: "{{ route('kartu-in-transit.get-mutasi-masuk') }}?month={{ $month }}&year={{ $year }}",
                    method: "GET",
                    success: function(res) {
                        console.log(res);
                        if (res.status == 1) {
            ...
            function linkJournal() {
                ...
                $.ajax({
                    ...
                    data: {
                        ...
                        "model_id": model_id,
                        "journal_id": id,
                        "model": "App\\Models\\KartuInTransit",
                    },
                    success: function(res) {
                        console.log(res);
                        if (res.status == 1) {
            ...
            function getMutasiKeluar() {
                page = "keluar";
                $.ajax({
                    url: "{{ route('kartu-in-transit.get-mutasi-keluar') }}?month={{ $month }}&year={{ $year }}",
                    method: "GET",
                    success: function(res) {
                        console.log(res);
                        if (res.status == 1) {
            ...
            function refreshKartu(id) {
                $.ajax({
                    url: "{{ route('kartu-in-transit.refresh-kartu') }}",
                    method: "POST",
                    data: {
                        "_token": "{{ csrf_token() }}",
                        "id": id,
