<table>
    @foreach ($data['coas'] as $code )
    <tr>
        <td colspan="8" style="font-size:12px;"><strong>{{ $code }} - {{ $data['chart_accounts'][$code] ?? '?' }} </strong></td>
    </tr>
    <tr>
    ...
    <tr>
        ...
        <td>{{ $item['lawan_code_group'] }} - {{ $data['chart_accounts'][$item['lawan_code_group']] ?? '?' }}
        </td>
