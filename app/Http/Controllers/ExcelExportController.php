...
class ExcelExportController extends Controller
{
    ...
    {
        $coas = ChartAccountAlias::aktif()->child()->where('code_group', '<', 120000)->orderBy('code_group')->pluck('code_group')->all();
        return self::_getMutationJournal($month, $year, $coas);
    }

    public static function getBukuMemo($month, $year, $singkat = false)
    {
        $coas = ChartAccountAlias::aktif()->child()->where('code_group', '>=', 120000)->orderBy('code_group')->pluck('code_group')->all();
        return self::_getMutationJournal($month, $year, $coas, $singkat);
    }

    public static function _getMutationJournal($month, $year, $coas, $singkat = false)
    {
        ...
        $journals = Journal::leftJoin('chart_account_aliases as lawan_code', function ($join) {
            $join->on('lawan_code.code_group', '=', 'journals.lawan_code_group')
                ->on('lawan_code.book_journal_id', '=', 'journals.book_journal_id');
        })->whereIn('journals.code_group', $coas)->whereMonth('journals.created_at', $month)->whereYear('journals.created_at', $year)
            ...
            ->select(
                ...
                'lawan_code.name as lawan_code_name',
                DB::raw('
                CASE WHEN journals.description LIKE "penjualan%" THEN concat("penjualan ",DATE_FORMAT(journals.created_at,"%Y-%m-%d"))             
                WHEN journals.description LIKE "ppn keluaran penjualan%" THEN concat("ppn keluaran penjualan ",DATE_FORMAT(journals.created_at,"%Y-%m-%d"))
                ELSE "lainnya" 
                END as type')
            )
        ...
        if ($singkat == true) {
            $journals = collect($journals)->map(function ($vals) {
                $alldatas = collect($vals)->groupBy('type')->map(function ($items, $type) {
                    if (preg_match('/penjualan/i', $type)) {
                ...
                return $alldatas->flatten(1);
            });
        }
        $chartAccount = ChartAccountAlias::aktif()->pluck('name', 'code_group');
        foreach ($coas as $coa) {
    ...
    public function renewReport($month, $year)
    {
        $month = $month ?? date('m');
        $year = $year ?? date('Y');
    }

    public function downloadMultiSheetReport($month, $year)
    {
        //ini buat download file yang sudah ada
    }
}
