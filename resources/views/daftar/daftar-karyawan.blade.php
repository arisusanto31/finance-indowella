        ...
        <div class="card shadow-sm mb-4">
            ...
            <div class="container py-4 p-3 mb-4 card shadow-sm">
                ...
                <div class="card-body">
                    <div class="row mt-1">
                        <div class="col-md-2">
                            <a href="#" onclick="ShowModalKaryawan()"
                                class="btn btn-primary btn-big-custom rounded-0">Create Karyawan</a>
                        </div>
                    </div>
                            ...
                            @foreach ($karyawans as $karyawan)
                                <tr>
                                    ...
                                    <td class="border px-4 py-2">
                                        ...
                                        @else
                                            <span class="text-sm text-gray-500">Keluar
                                                ({{ $karyawan->date_keluar }})
                                            </span>
                                        @endif
                                    </td>
                                    ...
                                    <td class="border px-4 py-2 space-x-1">
                                        ...
                                        {{-- <form action="{{ route('karyawan.karyawans.soft-delete', $karyawan->id) }}"
                                            method="POST" onsubmit="return confirm('Yakin mau hapus data ini?')">
                                            @csrf
                                            <button type="submit" class="btn btn-danger btn-sm">Delete</button>
                                        </form>  --}}

                                        <form id="soft-delete-form-{{ $karyawan->id }}" action="{{ route('karyawan.karyawans.soft-delete', $karyawan->id) }}" method="POST">
                                            @csrf
                                        </form>
                                        
                                        <button type="button" class="btn btn-danger btn-sm" onclick="confirmSoftDelete({{ $karyawan->id }})">
                                            Delete
                                        </button>
                                        
                                        @if (!$karyawan->date_keluar || $karyawan->date_keluar === '0000-00-00')
                                            <form action="{{ route('karyawan.resign', $karyawan->id) }}" method="POST"
                                                class="d-inline">
                                                @csrf
                                                @method('PUT')
                    ...
                    @push('scripts')
                        ...
                        <script>
                            function confirmSoftDelete(id) {
                                Swal.fire({
                                    title: 'Yakin mau hapus data ini lurr?',
                                    text: "Data akan delete!",
                                    icon: 'warning',
