...
class KartuDPSalesController extends Controller
{
    ...
    public function getSummary()
    {
        ...
        $kartuPiutangAwal = KartuDPSales::whereIn('id', function ($q) use ($date) {
            $q->from('kartu_dp_sales')->select(DB::raw('max(id)'))->where('created_at', '<', $date . '-01')->groupBy('package_number');
    ...
    public function showDetail($nomer)
    {
        ...
        $view->factur = $nomer;
        $kh = KartuDPSales::where('package_number', $nomer)->orderBy('created_at', 'desc')->first();
        $view->person = $kh->person;
        $data = KartuDPSales::where('package_number', $nomer)->get();
        $view->data = $data;
        return $view;
    }
