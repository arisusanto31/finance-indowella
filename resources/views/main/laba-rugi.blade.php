<x-app-layout>
    ...
    @push('scripts')
        <script>
            ...
            function _tampilkanBaris(kodePerk, string, amount, isStrong) {
                if (isStrong) {
                    stringStrong = "<strong>";
            ...
            function tampilkan(res) {
                if (res.status == 1) {
                    ...
                    html += _tampilkanBaris("", "Pendapatan Netto", formatRupiah(totalPenjualan), true);
                    ...
                    html += _tampilkanBaris("", "Total Beban Pokok", formatRupiah(totalBeban), true);

                    html+='<div style="margin-top:20px"></div>';
                    labaKotor= totalPenjualan-totalBeban;
                    html += _tampilkanBaris("", "LABA KOTOR", formatRupiah(labaKotor), true);
                    ...
                    html += _tampilkanBaris("", "LABA OPERASIONAL", formatRupiah(labaOperasional), true);
                    ...
                    html += _tampilkanBaris("", "Total Pendapatan lain", formatRupiah(totalPendapatanLain), true);
                    html += _tampilkanBaris("", "BEBAN LAIN", "", true);
                    bebanLain.forEach(function eachNeraca(lajur) {
                        isStrong = lajur.level < 1;
                        html += _tampilkanBaris(lajur.code_group, lajur.name, formatRupiah(lajur.saldo_akhir), isStrong);
                    });
                    html += _tampilkanBaris("", "Total Beban lain", formatRupiah(totalBebanLain), true);
                    html += _tampilkanBaris("", "TOTAL Pendapatan dan Beban Lain", formatRupiah(res.laba_bulan), true);
