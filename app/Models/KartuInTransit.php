...
class KartuInTransit extends Model
{
    
    use HasIndexDate;
    use HasModelSaldoStock;
    use HasModelDetailKartuInvoice;
    protected $table = 'kartu_in_transits';
    public $timestamps = true;

    protected static function booted()
    {
        static::addGlobalScope('journal', function ($query) {
            $from = $query->getQuery()->from ?? 'kartu_in_transits'; // untuk dukung alias `j` kalau pakai from('journals as j')
    ...
    public static function mutationStore(Request $request, $useTransaction = true, ?LockManager $lockManager = null)
    {
        ...
        try {
            ...
            if ($qty == 0) {
                throw new \Exception('qty kartu BDP tidak boleh nol. detail:' . json_encode($request->all()));
            }
            ...
            if (!$stockid && $customStockName) {
                ...
                $modeStock = 'custom';
                $kartuBDP = KartuInTransit::where('production_number', $productionNumber)->where('custom_stock_name', $customStockName)->first();
                if ($kartuBDP) {
                    $stockid = $kartuBDP->stock_id;
                } else {
                    $idCustom = Stock::withoutGlobalScope('journal')->where('name', 'like', 'stock_custom%')->pluck('id')->all();

                    $stockCustomTerpakai = KartuInTransit::where('production_number', $productionNumber)->whereIn('stock_id', $idCustom)->pluck('stock_id')->all();
                    $stockid = $idCustom[0];
                    $i = 0;
            ...
            if ($isOtomatisJurnal) {
                ...
                $journal = Journal::where('journal_number', $number)->where('code_group', 140002)->first();
    ...
    {
        $kartus = KartuInTransit::where('stock_id', $this->stock_id)->where('index_date', '>', $this->index_date)->where('production_number', $this->production_number)
